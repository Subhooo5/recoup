...
import { diagnoseFailedSubscription } from "@/lib/pipelines/failed-subscription/agent";
import { qualifyFailedSubscription } from "@/lib/pipelines/failed-subscription/trigger";
...
const PIPELINE = "failed-subscription" as const;
...
export const runFailedSubscription = async (
  subscription: Subscription,
): Promise<PipelineRunResult> => {
  const qualification = await qualifyFailedSubscription(subscription);
  ...
  await appendAuditLog({
    ...
    detail: {
      ...
      amount: qualification.amount,
      amountResolution:
        qualification.amountResolutionDetail ??
        "Resolved from the Razorpay plan",
      created,
    },
  });
  ...
  try {
    ...
    const agentOutput = await diagnoseFailedSubscription(context);
